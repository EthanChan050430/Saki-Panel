...
export function SakiDesktopPet({
  ...
  useLayoutEffect(() => {
    ...
    const mutationObserver = new MutationObserver(() => place("lock"));
      ...
      >
        <div className="saki-pet-nameplate">
          ...
          <div className="saki-pet-stats">
            <StatBar label={isEn ? "Hunger" : "饱食"} value={pet.stats.hunger} tone="hunger" />
            <StatBar label={isEn ? "Mood" : "心情"} value={pet.stats.mood} tone="mood" />
            <StatBar label={isEn ? "Health" : "健康"} value={pet.stats.health} tone="health" />
          </div>
        </div>
          ...
          <div className="saki-pet-tray companion">
            ...
            <TrayChip
              ...
              onClick={() => {
                pet.applyCare("pet");
                onIntimacy?.(3);
                pet.showBubble(isEn ? "Hehe, that tickles～" : "嘿嘿，再摸摸～");
              }}
            />
            <TrayChip
              icon={<Moon size={11} />}
              ...
              onClick={() => {
                pet.applyCare("sleep");
                onIntimacy?.(2);
                pet.showBubble(isEn ? "Napping… zzz" : "我先睡一会儿… zzz");
              }}
            />
            <TrayChip
              icon={<Bath size={11} />}
              ...
              onClick={() => {
                pet.applyCare("bath");
                onIntimacy?.(4);
                pet.showBubble(isEn ? "All clean!" : "洗香香啦～");
              }}
            />
            <TrayChip
              icon={<Stethoscope size={11} />}
              label={isEn ? "Doctor" : "看病"}
              onClick={() => {
                pet.applyCare("doctor");
                onIntimacy?.(2);
                pet.showBubble(isEn ? "Full HP!" : "已经活力满满！");
              }}
            />
            <TrayChip
