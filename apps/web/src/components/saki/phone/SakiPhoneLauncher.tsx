...
interface SakiPhoneLauncherProps {
  onSelectGame: (gameId: "dessert_drop" | "sweet_match" | "plant_slayer") => void;
  onClose: () => void;
...
export function SakiPhoneLauncher({
  ...
  useEffect(() => {
    const updateClock = () => {
      ...
      setCurrentDate(isEn ? `${dayName} · ${month}/${date}` : `${month}月${date}日 ${dayName}`);
    };
    updateClock();
  ...
  const galleryItems = [
    { name: isEn ? "Snack Time" : "品尝甜点", img: "/assets/expression/eating.webp", quote: "草莓大福好好吃～" },
    { name: isEn ? "Happy Wink" : "眨眼微笑", img: "/assets/expression/wink.webp", quote: "今天也是元气满满的一天！" },
    { name: isEn ? "Gamer Saki" : "沉迷游戏", img: "/assets/expression/gaming.webp", quote: "看我的高分神操作！" },
    { name: isEn ? "Warm Hug" : "开心陪伴", img: "/assets/expression/happy.webp", quote: "最喜欢和主人在一起了～" }
  ];
        ...
        <div className="saki-phone-widget-row">
          <div className="saki-phone-clock-widget">
            ...
            <span className="saki-widget-status">
              {isEn ? "SakiOS 4.0 · Clear sky" : isTw ? "星夢OS 4.0 · 晴空萬里" : "星梦OS 4.0 · 晴空万里"}
            </span>
          </div>
        ...
        <div className="saki-app-grid">
          ...
          >
            ...
            <span className="saki-app-label">
              {isEn ? "Dessert Drop" : isTw ? "接甜點" : "接甜点"}
            </span>
          </button>
          ...
          >
            ...
            <span className="saki-app-label">
              {isEn ? "Plant Slayer" : isTw ? "植物特攻" : "植物特攻"}
            </span>
          </button>
        ...
        <div className="saki-phone-easter-popup">
          <div className="saki-easter-header">
            <span className="saki-easter-title">
              ...
              <span>{isEn ? "Saki's Memories" : isTw ? "Saki 的回憶相冊" : "Saki 的回忆相册"}</span>
            </span>
            <button
