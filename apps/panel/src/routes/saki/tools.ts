...
export const sakiToolSchemas: SakiToolSchema[] = [
  ...
  { name: "runCommand", description: "Run a terminal command. IMPORTANT: Agent terminal commands ALWAYS execute in a NEWLY CREATED independent shell (exactly like clicking the + button in the UI terminal to open a fresh tab). This isolates from the main instance process. Not for the running console stdin (use sendInput/sendCommand for that). For multi-step in same shell, use createShell then runInShell. Medium/high risk need approval. Supports input for prompts.", parameters: objectSchema({ instanceId: instanceLookupSchema, command: { type: "string" }, cwd: { type: "string", description: "Optional subdirectory relative to the selected instance working directory." }, workingDirectory: { type: "string", description: "Alias for cwd; must be relative to the selected instance working directory." }, timeoutMs: { type: "integer", minimum: 1000, maximum: 120000 }, input: { type: "string" }, stdin: { type: "string" } }, ["command"]), aliases: ["executeCommand", "terminal", "shell"] },
