...
export const sakiToolSchemas: SakiToolSchema[] = [
  ...
  { name: "listShells", description: "List persistent shell tabs/sessions for the instance. These are independent long-lived shells created via the + button in the terminal UI.", parameters: objectSchema({ instanceId: instanceLookupSchema }) },
  { name: "createShell", description: "Create a new persistent independent shell session (like clicking + in terminal UI). Returns shellId for use with other shell tools. Use for stateful or long-running terminal work.", parameters: objectSchema({ instanceId: instanceLookupSchema, workingDirectory: { type: "string", description: "Optional relative cwd." } }) },
  { name: "sendShellInput", description: "Send raw input (keystrokes) to a specific persistent shell session. For commands, prefer runInShell or append \\n.", parameters: objectSchema({ instanceId: instanceLookupSchema, shellId: { type: "string" }, text: { type: "string" }, pressEnter: { type: "boolean" } }, ["shellId", "text"]) },
  { name: "runInShell", description: "Run a command in an existing persistent shell (created by createShell or + button). Better than runCommand for multi-step or stateful sessions.", parameters: objectSchema({ instanceId: instanceLookupSchema, shellId: { type: "string" }, command: { type: "string" }, timeoutMs: { type: "integer" } }, ["shellId", "command"]) },
