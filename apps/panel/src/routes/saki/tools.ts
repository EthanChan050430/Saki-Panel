...
export const sakiToolSchemas: SakiToolSchema[] = [
  ...
  { name: "extractArchive", description: "Extract a .zip, .rar, or .7z archive into the instance workspace. Prefer this over shell unzip commands. Set overwrite=true to replace an existing output directory.", parameters: objectSchema({ instanceId: instanceLookupSchema, path: relativePathSchema, outputPath: { type: "string", description: "Optional output directory relative to the instance working directory." }, overwrite: { type: "boolean", description: "Replace the output directory if it already exists." } }, ["path"]), aliases: ["extract", "unzipArchive", "decompressArchive"] },
